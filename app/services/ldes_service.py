...
class LdesService:
    ...
    def get_ldes_collection(self, collection_alias:str) -> Graph:
        graph = self._bind_namespaces(Graph())
        ldes = self.storage_provider.get_ldes_collection_by_alias(collection_alias)
    ...
    def get_ldes_node(
        ...
        try:
            view = Enumerable(ldes.views).single(lambda v: v.alias == view_alias)
        except NoMatchingElement as error:
            raise LdesNotFoundError(f"No view found with alias {view_alias} in collection {collection_alias}.")
        ...
        if not(view.fragmentation_kind) \
            ...
            page_fragmentation = PageFragmentation(graph, self.storage_provider, self.base_uri)
            ...
            raise LdesServerError(f"Fragmentation kind specified in view {view.id} is not supported by this server.")
    ...
    def create_or_update_ldes_collection(
        ...
        ldes = self.storage_provider.create_or_update_ldes_collection(collection_ref, alias, title, member_frame)
        return ldes
    ...
    def _add_collection_to_graph(self, graph: Graph, collection: TreeCollection):
        ...
        graph.add((collection_ref, PYLDES.memberFrameSpec, Literal(collection.member_frame)))
        return graph
