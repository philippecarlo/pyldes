...
class PageFragmentation:
    ...
    def __init__(self, graph: Graph, storage_provider, base_uri: str):
        ...
        self.graph = graph
        self.collection_ref = Enumerable(
            self.graph.subjects(RDF.type, LDES.EventStream)).single()
        self.collection_alias = Enumerable(self.graph.objects(
            self.collection_ref, PYLDES.alias)).single()
        if not (self.collection_ref):
            raise LdesServerError(
                "Internal error: No collection specification passed to PageFragmentation module.")
        self.view_description_ref = Enumerable(
            self.graph.subjects(RDF.type, TREE.ViewDescription)).single()
        self.view_alias = Enumerable(self.graph.objects(
            self.view_description_ref, PYLDES.alias)).single()
        if not (self.view_description_ref):
            raise LdesServerError(
                "Internal error: No view description passed to PageFragmentation module.")

        self.path = self.graph.value(self.view_description_ref, TREE.path)
        self.sequence_type = self.graph.value(
            self.view_description_ref, PYLDES.sequenceType)
        self.max_node_size = int(self.graph.value(
            self.view_description_ref, PYLDES.maxNodeSize))
    ...
    def get_node(self, node_id) -> Graph:
        ...
        node_ref = URIRef(
            f"{self.base_uri}/ldes/{self.collection_alias}/{self.view_alias}?node={node_id}")
        self.graph.add((node_ref, RDF.type, TREE.Node))
        self.graph.add((node_ref, TREE.viewDescription,
                       self.view_description_ref))

        self._add_node_members(node_id)
        min, max = self.get_fragment_min_max_value()

        if not (node_id == last_node):
            previous_relation = BNode()
            self.graph.add((previous_relation, RDF.type,
                           TREE.GreaterThanRelation))
            self.graph.add((previous_relation, TREE.path, self.path))
            self.graph.add((previous_relation, TREE.node, URIRef(
                f"{self.base_uri}/ldes/{self.collection_alias}/{self.view_alias}?node={node_id+1}")))
        ...
        if node_id > 1:
            ...
            self.graph.add((next_relation, TREE.node, URIRef(
                f"{self.base_uri}/ldes/{self.collection_alias}/{self.view_alias}?node={node_id-1}")))
    ...
    def _add_node_members(self, node_id):
        skip = (node_id - 1) * self.max_node_size
        skip = 0 if skip < 0 else skip
        members = self.storage_provider.get_ldes_members(
            self.collection_ref, self.path, self.sequence_type.toPython(), skip, self.max_node_size)
        buffer = str()
        for member in members:
            self.graph.add(
                (self.collection_ref, TREE.member, URIRef(member.id)))
            buffer += member.rdf
            buffer += os.linesep
    ...
    def get_last_node(self):
        member_count = self.storage_provider.get_ldes_member_count(
            self.collection_ref)
